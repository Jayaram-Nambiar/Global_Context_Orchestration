...
- **2026-09-25:** `.agent-context.json` is generated and gitignored. `meta.root` stays an absolute workspace path because MCP clients must pass it back as `workspace_path`. Do not commit the file. The project is MIT-licensed; do not describe it as an internal-only utility.
...
Follow `AGENTS.md`. After non-trivial MCP or installer changes, run `python -m unittest discover -s tests -v` and `ctx check` from the repo root. Redeploy with `scripts/deploy.ps1` so `~/.agent-context-engine` matches `src/`. Cursor only loads the **installed** copy, not the git working tree.
